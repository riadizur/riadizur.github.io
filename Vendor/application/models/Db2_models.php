...
class Db2_models extends CI_Model
{
	private $temp = array();
	function __construct() 
	{
		parent::__construct();
		$this->db2 = $this->load->database('epi_vendor', TRUE);
	}
	public function max_id_vendor(){
		$query = $this->db2->query("SELECT max(substr(kode_vendor FROM 11 FOR 3)) FROM master_perusahaan");
	...
	public function list_dropdown($nama_tabel, $data, $where, $adder = '', $lainnya='',$placeholder='--Pilih--')
	{
		...
		if ($where == 'all') {
			$query = $this->db2->query("SELECT $data[0] as id, $uraian as uraian FROM $nama_tabel $lainnya");
		} else {
			$query = $this->db2->query("SELECT $data[0] as id, $uraian as uraian FROM $nama_tabel $where $lainnya");
		}
	...
	public function list_dropdown_resultx($nama_tabel, $data, $where, $adder = '', $lainnya='')
	{
		...
		if ($where == 'all') {
			...
			$query = $this->db2->query("SELECT $data[0] as id, $uraian as uraian FROM $nama_tabel $whereis $lainnya");
		}
		$dropdowns = $query->result();
		return $dropdowns;
	}
	...
	public function cek($tabel,$where){
		$query=$this->db2->get_where($tabel,$where);
		if($query->num_rows()>0){
			return TRUE;
		}else{
			return FALSE;
	...
	public function get_data($tabel,$data,$where){
		...
		if ($where == 'all') {
			$query = $this->db2->query("SELECT $kolom FROM $tabel");
		} else {
			$query = $this->db2->query("SELECT $kolom FROM $tabel where kode='$where'");
		}
		$hasil = $query->result();
		return $hasil;
	}
	...
	{
		$this->db2->update($tabel, $data, $where);
	}
	public function delete($tabel, $where)
	{
		$this->db2->delete($tabel, $where);
	}
	public function result($nama_tabel, $where)
	{
		if($where!='all'){
			$query = $this->db2->get_where($nama_tabel, $where);
		}else{
			$query = $this->db2->get($nama_tabel);
		}
		return $query->result();
	}
	...
	{
		if($where!='all'){
			$query = $this->db2->distinct()->select($distinct)->get_where($nama_tabel, $where);
		}else{
			$query = $this->db2->distinct()->select($distinct)->get($nama_tabel);
		}
		return $query->result();
	}
	...
	public function sum($tabel,$kolom,$where){
		$query = $this->db2->query("SELECT sum($kolom) as data from $tabel WHERE $where");
		return $query->row('data');
	}
	public function max($tabel,$kolom,$where){
		$query = $this->db2->query("SELECT max($kolom) as max from $tabel WHERE $where");
		return $query->row('max');
	}
	public function next_id_new($tabel,$kolom, $where, $code_position, $index_position, $master_id){
		$query = $this->db2->query("select if(max(if(left($kolom,$index_position)='$master_id',right($kolom,2),''))+1<10,
	...
	public function next_index($tabel,$kolom, $where, $code_position, $index_position, $master_id){
		$query = $this->db2->where($where)->query("select max($code_position($kolom,$index_position)='$master_id')+1 as $kolom from $tabel");
		return $query->row($kolom);
	}
}