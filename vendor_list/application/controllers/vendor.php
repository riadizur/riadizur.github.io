...
class Vendor extends CI_Controller {
	...
	public function entri(){
		if($this->session->userdata('nama')<>''){
			...
			$datax['list_grade']=$this->referensim->get_grade_list();
			$datax['list_siujk']=$this->referensim->get_siujk_detil_list();

			$this->load->view('vendor/entri',$datax);
		}else{
	...
	public function list_siujk_insert($id_vds, $kodex, $kode_grade)
	{
		$datax = array();
		...
		if($cek > 0){
			...
			$data_siujk['grade_sub_siujk'] = $kode_grade;
	...
	public function vendor_list_1()
	{
		...
		foreach ($list as $pelayanan) {
			...
			$row[] = $pelayanan->grade_siujk;
			$row[] = $pelayanan->nama_pic;
			$row[] = $pelayanan->no_hp_pic;
			$row[] = $pelayanan->email_pic;
