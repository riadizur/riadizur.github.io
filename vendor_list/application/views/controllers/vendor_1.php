...
class Vendor extends CI_Controller {
	...
	public function entri(){
		if($this->session->userdata('nama')<>''){
			...
			$this->load->view('vendor/entri_3',$datax);
		}else{
	...
	public function vendor_add()
	{
		...
		foreach ($a as $r)
		{
			...
			{
				$id_1 = substr($this->input->post($r->COLUMN_NAME),0,7);
		...
		if ($this->input->post('nomor_siujk') <> '')
		{
			$b = $this->referensim->get_tvd_list_siujk();
			$nomor_siujk = $this->input->post('nomor_siujk');
			$data_list['nomor_siujk'] = strtolower($this->input->post('nomor_siujk'));
			foreach ($b as $r)
			{
				$cek = substr($r->COLUMN_NAME,0,4);
				switch ($cek)
				{
					case 'file':
						if(empty($_FILES[$r->COLUMN_NAME]['name']))
						{
							$datax['inputerror'][] = $r->COLUMN_NAME;
							$datax['error_string'][] = 'File tidak boleh kosong';
							$datax['status'] = FALSE;
						}
						else
						{
						...
						break;
					default:
						if (empty($this->input->post($r->COLUMN_NAME)))
						{
							$datax['inputerror'][] = $r->COLUMN_NAME;
							$datax['error_string'][] = 'File tidak boleh kosong';
							$datax['status'] = FALSE;
		...
		if ($this->input->post('nomor_siu') <> '')
		{
			$c = $this->referensim->get_tvd_list_siu();
			$nomor_siu = $this->input->post('nomor_siu');
			$data_list['nomor_siu'] = $this->input->post('nomor_siu');
			foreach ($c as $r)
			{
				$cek = substr($r->COLUMN_NAME,0,4);
				switch ($cek)
		...
		if($nomor_siup <> '')
		{
			if($b <> '' )
			{
				for ($i = 1; $i <= $b; ++$i)
				{
					...
					{
						$ids = $this->input->post($nama);
						$id_data = $this->referensim->get_kbli_detil_id($ids);
						$data_siup['kode_siup'] = $id_data->kode;
						$data_siup['bidang_siup'] = $id_data->deskripsi;
						$data_siup['id_vd'] = $id_vds;

						$insert_data_siup = $this->referensim->save('tvd_siup',$data_siup);
					}
				}
			}
			else
			{
				$datax['inputerror'][] = 'notif_siup';
				$datax['error_string'][] = 'Error : Inputan kode KBLI tidak boleh kosong';
				$datax['status'] = FALSE;
			}
		}
		...
		if($nomor_siujk <> '')
		{
			if($c <> '' )
			{
				for ($i = 1; $i <= $c; ++$i)
				{
					...
					{
						$ids = $this->input->post($nama);
						$id_data = $this->referensim->get_siujk_detil_id($ids);
						$data_siujk['kode_siujk'] = $id_data->kode_sub;
						$data_siujk['bidang_siujk'] = $id_data->sub_klasifikasi;
						$data_siujk['id_vd'] = $id_vds;

						$insert_data_siujk = $this->referensim->save('tvd_siujk',$data_siujk);
					}
				}
			}
			else
			{
				$datax['inputerror'][] = 'notif_siujk';
				$datax['error_string'][] = 'Error : Inputan kode Klasifikasi tidak boleh kosong';
				$datax['status'] = FALSE;
			}
		}
		...
		if($nomor_siu <> '')
		{
			if($d <> '')
			{
				for ($i = 1; $i <= $d; ++$i)
				{
					...
					if ($this->input->post($nama) <> '')
					{
						$data_siu['bidang_siu'] = $this->input->post($nama);
						$data_siu['id_vd'] = $id_vds;

						$insert_data_siu = $this->referensim->save('tvd_siu',$data_siu);
					}
				}
			}
			else
			{
				$datax['inputerror'][] = 'notif_siu';
				$datax['error_string'][] = 'Error : Inputan Klasifikasi tidak boleh kosong';
				$datax['status'] = FALSE;
			}
		}
