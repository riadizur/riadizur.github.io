...
class Pengadaan extends CI_Controller {
	...
	public function reg_project(){ 
		if ($this->session->userdata('nama') <> '') {
			...
			$list_tahun_project=$this->db3_models->list_dropdown('tkontrak',array('distinct(tahun_kontrak)','tahun_kontrak'),'order by tahun_kontrak desc','','','Select');
			$load_project='list_dropdown3(' . "'dropdown_project',[],'tkontrak',['id_kontrak','nama_pekerjaan'],'where tahun_kontrak='+$(this).val(),'','','Select'" . ')';
			$data['dropdown_tahun_project']=form_dropdown("dropdown_tahun_project", $list_tahun_project, [], 'id="dropdown_tahun_project" onchange="' . $load_project . '" style="width:100%; " class="form-control select2me"');
			$data['dropdown_project']=form_dropdown("dropdown_project", 'Select', [], 'id="dropdown_project" onchange="load_data_project($(this).val());" style="width:100%; " class="form-control select2me"');
			$data['nama_dropdown_tahun_project'] = 'dropdown_tahun_project';
			$data['nama_dropdown_project'] = 'dropdown_project';
			...
			$list_jen_pekerjaan=$this->db_models->list_dropdown('tr_bidang_pekerjaan',array('kode','nama'),'','','','Select');
	...
	private function tabel_publish($where){
		...
		if($where!='all'){
			if(array_key_exists('mode', $where)){ 
				unset($where['mode']);
				$kode_kbli_all = $this->db_models->row('tp_master_project',$where,'kode_bidang_pekerjaan');
				$kode_kbli = explode(", ", $kode_kbli_all);
				$filter='';
				...
				if($size>1){
					for ($x=0; $x<$last;$x++) {
						$filter .= 'kode_bidang=\'' . $kode_kbli[$x] .'\' or ';
					}
				}
				$hasil = $this->db_models->result_distinct('master_bidang_pekerjaan',array($filter . 'kode_bidang_pekerjaan='=>$kode_kbli[$last]),'kode_reg');
			}else{
				$kode_kbli_all = $this->db_models->row('tp_master_project',$where,'kode_bidang_pekerjaan');
				$kode_kbli = explode(", ", $kode_kbli_all);
				$hasil = $this->db_models->result_distinct('master_bidang_pekerjaan',array('kode_bidang='=>$kode_kbli[0]),'kode_register');
			}
		}else{
		...
		foreach ($hasil as $r) {
			...
			$row[] = $i;
			if($where!='all'){
				$row[] = $this->db_models->row('master_perusahaan',array('kode_register'=>$r->kode_register),'nama_perusahaan');
			}else{
	...
	public function publish(){
		...
		$kode_kbli_all = $this->db_models->row('tp_master_project',array('keperluan'=>$keperluan,'kode_project'=>$kode_project),'kode_bidang_pekerjaan');
		$kode_kbli = explode(", ", $kode_kbli_all);
		$filter='';
		$size=sizeof($kode_kbli);
		$last=$size-1;
		...
		switch($mode){
			case 'sesuai_kbli' : $kode_reg = $this->db_models->result_distinct('master_ijin',array('kode_kbli='=>$kode_kbli[0]),'kode_reg');
								 ...
								 break;
			case 'sesuai_bidang_pekerjaan' : $kode_reg = $this->db_models->result_distinct('master_bidang_pekerjaan',array('kode_bidang='=>$kode_kbli[0]),'kode_register');
								 foreach ($kode_reg as $r) {
									$hasil[]= $this->db_models->result('master_perusahaan',array('kode_register'=>$r->kode_register));						 
								 } 
								 break;
	...
	public function load_data(){
		...
		$data = $this->input->post('data');
		switch($data){
			case 'project' : $data_tabel=$this->db3_models->result('tkontrak',array('id_kontrak'=>$filter)); break;
		}
